module MessageApis::GoBot
  class Presenter
    ...
    def self.initialize_hook(kind:, ctx:)
      record = MessageApis::GoBot::PromptRecord.new(prompt: ctx.dig(:values, :prompt))
      {
        kind: kind,
        # ctx: ctx,
    ...
    def self.submit_hook(kind:, ctx:)
      if ctx["field"]["id"] == "prompt-ok"
        ...
        conversation.conversation_channels.create({
                                                    provider: MessageApis::GoBot::Api::PROVIDER,
    ...
    def self.configure_hook(kind:, ctx:)
      ...
      record = MessageApis::GoBot::PromptRecord.new(prompt: value)
  ...
  class PromptRecord
    ...
    def default_schema
      [
        { type: "text", text: "GO chatbot", style: "header" },
    ...
    def success_schema
      [
        { type: "text", text: "GO chatbot conversation", style: "header" },
        { type: "text", text: "You are going to start a conversation with CIENCE chatbot", style: "muted" },
