  ...
  function Input ({
    ...
    function renderText () {
      return <FormField name={name}
        ...
        <input
          className={`shadow appearance-none border border-${borderColor(error)}-500 
    ...
    function renderSelect () {
      const initialValue = props.data && props.data.multiple
    ...
    function renderTextArea () {
      return (
        <div className="mt-6">
          {label && <label htmlFor="about" className="block text-sm leading-5 font-medium text-gray-700">
            {label}
          </label>}
          <div className="">
            <textarea id="about" rows="3"
              className={`shadow appearance-none border border-${borderColor(error)}-500 rounded 
          ...
          {helperText && <p className="mt-2 text-sm text-gray-500">
            {helperText}
          </p>}
        </div>
      )
    ...
    function renderTimezone () {
      const names = props.options.map(o => (
        { label: o, value: o }
      ))
    ...
    return <div className={`mb-4 ${props.className ? props.className : ''}`}>
      {
        handeRenderType()
      }
    </div>
