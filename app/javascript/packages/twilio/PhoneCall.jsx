...
export const PhoneCall = ({ data, endpointURL, i18n }) => {
  ...
  const { conversation_key, agents_id, profile_id, user_key, user } = data;
  ...
  const toggleHold = useCallback(async () => {
    var data = {
      ...
      hold_action: callState === CallStates.OnHold ? false : true,
    };
  ...
  }, [endpointURL, callState, conversation_key, profile_id]);

  const hangUpCall = useCallback(async () => {
    device.__activeConnection.disconnect();

    // Remove hold before hanging up so.
    if (callState === CallStates.OnHold) {
      toggleHold();
    }
      ...
      if (event.payload.FriendlyName == conversation_key) {
        switch (event.payload.StatusCallbackEvent) {
          case 'participant-hold':
            setCallState(CallStates.OnHold);
            break;
